...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (trip.trip_status !== 'PAUSED') {
      return apiError(`Impossible de reprendre: statut ${trip.trip_status}`, 409)
    }

    await db.execute(sql`
      UPDATE taxi_trips SET trip_status = 'RESUMED', updated_at = now() WHERE id = ${trip.id}
    `)
    await db.execute(sql`
      ...
      VALUES (gen_random_uuid(), ${trip.id}, ${trip.taximeter_id}, ${ctx.driverId}, 'WAITING_END', 'PAUSED', 'RESUMED', now(), now())
    `)
    return apiSuccess({ tripReference, status: 'RESUMED', resumedAt: new Date().toISOString() })
  } catch {

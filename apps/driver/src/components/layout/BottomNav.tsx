...
import { Home, Gauge, DollarSign, User, Grid } from 'lucide-react'
...
const menuItems = [
  ...
  { href: '/tax',           label: 'Centre fiscal',      emoji: '🧾', cat: 'fiscal'    },
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-black/80 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="absolute bottom-20 left-0 right-0 mx-3 rounded-2xl bg-slate-900 border border-slate-700 overflow-hidden shadow-2xl"
            onClick={e => e.stopPropagation()}>
            <div className="p-3 border-b border-slate-800">
              <div className="text-xs font-black tracking-[0.2em] text-qc-blue text-center">TAXIMETER.GOV</div>
              <div className="text-[9px] text-slate-500 text-center">Espace chauffeur</div>
            </div>
            <div className="max-h-[65vh] overflow-y-auto p-2">
              {Object.entries(cats).map(([cat, catLabel]) => (
                <div key={cat} className="mb-2">
                  <div className="text-[9px] font-bold uppercase tracking-widest text-slate-500 px-2 py-1">{catLabel}</div>
                  <div className="grid grid-cols-3 gap-1.5">
                    {menuItems.filter(i => i.cat === cat).map(item => {
                      const active = pathname === item.href
                      return (
                        <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                          className={`flex flex-col items-center gap-1 p-2.5 rounded-xl transition-all ${active ? 'bg-qc-blue text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-md border-t border-slate-800">
        <div className="flex items-center justify-around px-2 pt-2 pb-1">
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-all ${active ? 'text-qc-blue' : 'text-slate-500'}`}>
          ...
          <button onClick={() => setOpen(o => !o)}
            className={`flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-all ${open ? 'text-qc-blue bg-qc-blue/10' : 'text-slate-500'}`}>
            <Grid size={22} strokeWidth={open ? 2.5 : 1.5} />

    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={()=>setShowForm(!showForm)} className="px-3 py-2 rounded-xl text-sm font-bold bg-qc-blue text-white cursor-pointer hover:bg-blue-700 shrink-0">+ Soumettre</button>
        </div>
        ...
        {showForm&&(
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <label className="text-sm font-bold text-slate-500 block mb-1">Déclaration liée</label>
              ...
              <div>
                <label className="text-sm font-bold text-slate-500 block mb-1">Méthode de paiement</label>
                <select className="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs bg-white dark:bg-slate-900 text-slate-800 dark:text-white outline-none">
              ...
              {['Montant payé ($)','Numéro de référence','Date de paiement','Institution financière'].map(f=>(
                <div key={f}>
                  <label className="text-sm font-bold text-slate-500 block mb-1">{f}</label>
            ...
            <div className="text-sm text-red-500 text-center mt-2">PILOTE — Aucun paiement réel soumis</div>
          </div>
        )}
        ...
        {nextPay&&(
          <div className="bg-blue-600 text-white rounded-2xl p-5 shadow-sm flex items-center justify-between gap-4">
            <div>
              <div className="text-sm font-bold opacity-80 mb-1">📅 PROCHAIN PAIEMENT</div>
              <div className="text-xl font-black">{nextPay.type} — {nextPay.period}</div>
              <div className="text-sm opacity-75 mt-1">Échéance : {nextPay.dueDate} · Solde : {money2(nextPay.balance)}</div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-3xl font-black">{money(nextPay.due)}</div>
              <button onClick={()=>setShowForm(true)} className="mt-2 block text-sm font-bold bg-white text-blue-600 px-3 py-1.5 rounded-xl hover:bg-blue-50 cursor-pointer">→ Soumettre · DEMO</button>
            </div>
          </div>
        )}
              ...
              <div key={p.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm" style={{borderLeft:`3px solid ${sc.color}`}}>
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    ...
                    <div className="text-sm font-mono text-slate-400">{p.id}</div>
                    {p.ref&&<div className="text-sm font-mono text-green-600 dark:text-green-400">Réf: {p.ref}</div>}
                    {decl?.govRef&&<div className="text-sm font-mono text-blue-600 dark:text-blue-400">Réf. GOV: {decl.govRef}</div>}
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-2xl font-black text-slate-800 dark:text-white">{money2(p.due)}</div>
                    <div className="text-sm text-slate-400">Échéance: {p.dueDate}</div>
                  </div>
                </div>
                ...
                {p.status==='PAID'&&(
                  <div className="bg-green-50 dark:bg-green-500/8 border border-green-200 dark:border-green-500/20 rounded-xl p-3 mb-3">
                    ...
                    {p.notes&&<div className="text-sm text-slate-500 italic mt-1.5">{p.notes}</div>}
                  </div>
                )}
                {p.status==='UPCOMING'&&p.notes&&(
                  <div className="text-sm text-amber-600 dark:text-amber-400 italic mb-3">{p.notes}</div>
                )}

                {/* Pénalités */}
                {p.lateDays>0&&(
                  <div className="bg-red-50 dark:bg-red-500/8 border border-red-200 dark:border-red-500/20 rounded-xl p-3 mb-3">
                    <div className="text-sm font-bold text-red-600 dark:text-red-400">⚠️ Retard: {p.lateDays} jour(s) · Pénalité estimée: {money2(p.penaltyAmt)}</div>
                  </div>
                )}

                <div className="flex gap-2">
                  {p.declId&&<Link href="/declarations" className="px-3 py-1.5 rounded-xl text-sm font-bold bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100">📋 Déclaration liée</Link>}
                  <Link href="/obligations" className="px-3 py-1.5 rounded-xl text-sm font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100">📅 Obligation</Link>
        ...
        <div className="text-sm text-slate-400 text-center">{PILOT} · Paiements simulés · Aucun virement gouvernemental réel</div>
      </div>
    </AppShell>
  )
}

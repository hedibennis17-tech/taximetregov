    ...
    <AppShell>
      <div className="px-4 md:px-6 py-4 space-y-4 max-w-4xl mx-auto">
        ...
        <div className="rounded-2xl p-5 shadow-sm" style={{background:'#000'}}>
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm" style={{color:'rgba(255,255,255,0.5)'}}>{user.email}</div>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-sm font-bold px-2 py-0.5 rounded-full text-white" style={{background:color}}>{label}</span>
                <span className="text-sm font-bold text-green-400">🔒 SESSION SÉCURISÉE</span>
              </div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-sm" style={{color:'rgba(255,255,255,0.35)'}}>Enterprise ID</div>
              <div className="text-sm font-black text-white">{user.enterpriseId}</div>
              <div className="text-xs mt-1 text-amber-400">{PILOT}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
            ...
            {[
              ...
              <div key={r.l} className="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800 last:border-0 text-sm">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
            ...
            <div className="text-sm text-slate-400 mb-3">Basées sur le rôle : <span className="font-bold" style={{color}}>{label}</span></div>
            <div className="max-h-52 overflow-y-auto space-y-1 pr-1">
              {perms.map(p=>(
                <div key={p} className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0"/>
                  <span className="text-sm font-mono text-slate-600 dark:text-slate-400">{p}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
          ...
          {log.length === 0 ? (
            <div className="text-sm text-slate-400 italic">Aucun événement enregistré</div>
          ) : (
              ...
              {log.map((e:SecurityLogEntry)=>(
                <div key={(e as {id:string}).id} className="flex items-center gap-3 py-1.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-slate-800 dark:text-slate-200">{e.event}</div>
                    {e.detail&&<div className="text-sm text-slate-400 truncate">{e.detail}</div>}
                    {e.route&&<div className="text-sm font-mono text-slate-400">{e.route}</div>}
                  </div>
                  <div className="text-sm text-slate-400 shrink-0">{e.at.split('T')[1].slice(0,8)}</div>
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full shrink-0 ${e.success?'text-green-600 bg-green-50':'text-red-500 bg-red-50'}`}>
                    {e.success?'OK':'REFUSÉ'}
                  </span>
                </div>
              ))}

  ...
  return (
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-5xl mx-auto">
        ...
        <div className="text-sm font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-3 py-2 rounded-xl">{PILOT} · Anti-duplication: source + external_reference</div>

        {/* Workflow */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 shadow-sm">
          <div className="text-sm font-bold text-slate-400 uppercase mb-2">Chaîne activité → financier</div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm">
          <div className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Revenus par activité (DEMO)</div>
          <div className="flex items-end gap-1 h-16">
            {ALL_ACTIVITIES.map(a=>(
              <div key={a.id} className="flex-1 flex flex-col items-center gap-0.5">
                ...
                <div className="text-sm text-slate-400 whitespace-nowrap overflow-hidden">{a.id.split('-').pop()}</div>
              </div>
            ))}
          </div>
          <div className="flex gap-2 mt-2 flex-wrap">
            {Object.entries(PROVIDER_COLORS).map(([p,c])=>(
              <div key={p} className="flex items-center gap-1"><div className="w-2 h-2 rounded-sm" style={{background:c}}/><span className="text-sm text-slate-500">{p}</span></div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={()=>setTypeF('ALL')} className="px-2.5 py-1.5 rounded-xl text-sm font-bold border transition-all cursor-pointer" style={{background:typeF==='ALL'?'#003DA5':'transparent',color:typeF==='ALL'?'white':'#64748B',borderColor:typeF==='ALL'?'#003DA5':'rgba(148,163,184,0.30)'}}>Tous</button>
            {types.map(t=>(
              <button key={t} onClick={()=>setTypeF(t)} className="px-2.5 py-1.5 rounded-xl text-sm font-bold border transition-all cursor-pointer" style={{background:typeF===t?'#003DA5':'transparent',color:typeF===t?'white':'#64748B',borderColor:typeF===t?'#003DA5':'rgba(148,163,184,0.30)'}}>
                {ACT_TYPE_ICONS[t]??'📋'} {t}
              </button>
            ))}
            <select value={provF} onChange={(e: React.ChangeEvent<HTMLSelectElement>)=>setProvF(e.target.value)} className="px-2.5 py-1.5 rounded-xl text-sm font-bold border bg-transparent text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 outline-none cursor-pointer">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            <span className="text-sm text-amber-600 dark:text-amber-400">PILOTE · Anti-doublon actif</span>
          </div>
          {filtered.map(a=>{
              ...
              <div key={a.id} className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className="text-sm font-bold text-slate-800 dark:text-slate-200 font-mono">{a.id}</span>
                    <span className="text-sm font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{a.type}</span>
                    <span className="text-sm font-bold px-1.5 py-0.5 rounded" style={{color:PROVIDER_COLORS[a.provider]??'#64748B',background:'rgba(0,0,0,0.05)'}}>{a.provider}</span>
                  </div>
                  <div className="text-sm text-slate-400">{drv?.name??a.driverId} · {a.vehicleId} · {fmtDt(a.at)}</div>
                  <div className="text-sm text-slate-400">{a.origin} → {a.dest} · {a.dist}km · {a.dur}min{a.wait>0?` · Att: ${a.wait}min`:''}</div>
                  <div className="text-sm font-mono text-slate-400">Réf: {a.extRef} · TX: {a.txId}</div>
                </div>
                <div className="text-right shrink-0">
                  ...
                  <div className="text-sm text-slate-400">TPS: {money2(a.tps)}</div>
                  <div className="flex items-center gap-1 justify-end mt-0.5">
                    <div className={`w-1.5 h-1.5 rounded-full ${ss.dot}`}/>
                    <span className="text-sm font-bold" style={{color:ss.color}}>{ss.label}</span>
                  </div>
                </div>
              </div>
            )
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm">
          <div className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Par fournisseur</div>
          {providers.map(p=>{
            const pActs = ALL_ACTIVITIES.filter(a=>a.provider===p)
            const pGross = pActs.reduce((s,a)=>s+a.fare,0)
            return (
              <div key={p} className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800 last:border-0 text-sm">

      ...
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-5xl mx-auto">
        ...
        <div className="rounded-2xl p-5 shadow-sm" style={{background:'#000'}}>
          <div className="flex items-center gap-4 mb-4">
            <div className="shrink-0">
              ...
              <div className="text-[8px] font-bold mt-0.5" style={{color:'rgba(255,255,255,0.4)'}}>QUÉBEC · ENTERPRISE GOV · PILOTE</div>
            </div>
            <div className="flex-1">
              <div className="text-white font-black">{CURRENT_ENT.legalName}</div>
              <div className="text-[9px]" style={{color:'rgba(255,255,255,0.55)'}}>{CURRENT_ENT.tradeName} · NEQ: {CURRENT_ENT.neq}</div>
              <div className="flex items-center gap-1.5 mt-1">
                <div className="w-2 h-2 rounded-full bg-green-400"/>
                <span className="text-[8px] font-bold text-green-400">Connecté · PILOTE</span>
                <span className="text-[8px]" style={{color:'rgba(255,255,255,0.4)'}}>{activeDepts.length} départements actifs</span>
              </div>
            </div>
          ...
          <div className="flex gap-1.5 flex-wrap mb-3">
            <button onClick={()=>setDeptFilter('ALL')} className="text-[8px] font-bold px-2.5 py-1.5 rounded-lg cursor-pointer transition-all" style={{background:deptFilter==='ALL'?'white':'rgba(255,255,255,0.1)',color:deptFilter==='ALL'?'black':'rgba(255,255,255,0.7)'}}>
              Tous les départements
            </button>
            {activeDepts.map(d=>(
              <button key={d.slug} onClick={()=>setDeptFilter(d.slug)} className="text-[8px] font-bold px-2.5 py-1.5 rounded-lg cursor-pointer transition-all" style={{background:deptFilter===d.slug?d.color:'rgba(255,255,255,0.08)',color:deptFilter===d.slug?'white':'rgba(255,255,255,0.6)'}}>
                {d.emoji} {d.name}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-4 gap-2" style={{borderTop:'1px solid rgba(255,255,255,0.1)',paddingTop:'12px'}}>
            {[
              ...
              <div key={s.l}>
                ...
                <div className="text-[7px]" style={{color:'rgba(255,255,255,0.45)'}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
            ...
            <div className="text-[9px] text-slate-400 mb-3">Données synthétiques · PILOTE</div>
        ...
        {nextObl&&(
          <div className="rounded-2xl p-4 flex items-center justify-between gap-3" style={{background:'#000'}}>
            <div>
              <div className="text-[8px] font-bold mb-0.5" style={{color:'rgba(255,255,255,0.5)'}}>📅 PROCHAINE OBLIGATION</div>
              <div className="text-sm font-black text-white">{nextObl.type} — {nextObl.period}</div>
              <div className="text-[9px]" style={{color:'rgba(255,255,255,0.5)'}}>Échéance: {nextObl.due} · {money2(nextObl.amount)} · DEMO</div>
            </div>
          ...
          {ENT_CONNECTIONS.slice(0,5).map(c=>{
            ...
            return (
              <div key={c.id} className="flex items-center gap-3 py-1.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                ...
                <span className="text-[8px] font-bold" style={{color:cs.color}}>{cs.label}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="text-[8px] text-slate-400 text-center">⚠️ {CURRENT_ENT.revenusNote}</div>
      </div>
    </AppShell>
  )
}

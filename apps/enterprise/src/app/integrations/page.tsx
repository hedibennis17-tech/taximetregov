  ...
  return (
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-5xl mx-auto">
        ...
        <div className="text-sm font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-3 py-2 rounded-xl">{PILOT} · Métriques synthétiques · Aucune API externe réelle connectée</div>

        {/* Pipeline */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 shadow-sm">
          <div className="text-sm font-bold text-slate-400 uppercase mb-2">Pipeline de traitement</div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {([['apis','⚙️ APIs'],['webhooks','📡 Webhooks'],['scopes','🔑 Scopes'],['services','🚕 Services Uber']] as const).map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} className="px-3 py-2 rounded-xl text-sm font-bold border transition-all cursor-pointer" style={{background:tab===t?'#000000':'transparent',color:tab===t?'white':'#64748B',borderColor:tab===t?'#000000':'rgba(148,163,184,0.30)'}}>
              {l}
            </button>
          ))}
        </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span className="text-xs font-mono text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{api.version}</span>
                      ...
                      <div className="flex gap-1 flex-wrap mt-1.5">
                        {api.scopes.map(s=><span key={s} className="text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{s}</span>)}
                      </div>
                    </div>
                    {api.requests>0&&(
                    ...
                    {WEBHOOK_EVENTS_FULL.map(w=>{
                      ...
                      return (
                        <tr key={w.id} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                          <td className="px-3 py-2.5 font-mono text-sm text-blue-600 dark:text-blue-400 whitespace-nowrap">{w.event}</td>
                          <td className="px-3 py-2.5 text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">{w.src}</td>
                          <td className="px-3 py-2.5 text-sm text-slate-500 whitespace-nowrap">{w.dst}</td>
                          <td className="px-3 py-2.5 text-sm font-mono text-slate-400 whitespace-nowrap">{fmtDt(w.at)}</td>
                          <td className="px-3 py-2.5"><span className="text-sm font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap" style={{color:sc.color,background:sc.bg}}>{w.status}</span></td>
                          <td className="px-3 py-2.5 text-center text-sm">{w.attempts}</td>
                          <td className="px-3 py-2.5 text-sm font-mono text-slate-400">{w.traceId}</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              <div key={s.scope} className="flex items-start gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-sm font-mono font-bold text-black dark:text-white">{s.scope}</span>
                  </div>
                  <div className="text-sm text-slate-400">{s.desc}</div>
                </div>
                <div className="flex gap-1 flex-wrap justify-end shrink-0">
                  {s.roles.map(r=><span key={r} className="text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{r}</span>)}
                </div>
              </div>
            ))}
          </div>
        ...
        {tab==='services'&&(
          <div className="space-y-2">
            <div className="text-sm text-amber-600 dark:text-amber-400 italic px-1">Données DEMO — Répartition synthétique par service Uber</div>
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {UBER_SERVICES_DASHBOARD.map(s=>(
                      <tr key={s.id} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50" style={{opacity:s.status==='ACTIVE'?1:0.5}}>
                        <td className="px-3 py-2.5"><div className="flex items-center gap-2"><span className="text-lg">{s.emoji}</span><span className="text-sm font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap">{s.name}</span></div></td>
                        <td className="px-3 py-2.5"><span className={`text-sm font-bold px-1.5 py-0.5 rounded-full ${s.status==='ACTIVE'?'text-green-600 bg-green-50 dark:text-green-400 dark:bg-green-500/10':'text-purple-600 bg-purple-50 dark:text-purple-400 dark:bg-purple-500/10'}`}>{s.status}</span></td>
                        <td className="px-3 py-2.5 text-sm text-center font-bold text-slate-700 dark:text-slate-300">{s.drivers||'—'}</td>
                        <td className="px-3 py-2.5 text-sm text-center font-bold text-slate-700 dark:text-slate-300">{s.vehicles||'—'}</td>
                        <td className="px-3 py-2.5 text-sm text-center text-slate-600 dark:text-slate-400">{s.activities||'—'}</td>
                        <td className="px-3 py-2.5 text-sm text-center text-slate-600 dark:text-slate-400">{s.txCount||'—'}</td>
                        <td className="px-3 py-2.5 font-bold text-green-600 dark:text-green-400 whitespace-nowrap">{s.gross>0?money(s.gross):'—'}</td>
                        <td className="px-3 py-2.5 text-sm text-blue-600 dark:text-blue-400">{s.tips>0?money2(s.tips):'—'}</td>
                        <td className="px-3 py-2.5 text-sm text-purple-600 dark:text-purple-400">{s.tps>0?money2(s.tps):'—'}</td>
                        <td className="px-3 py-2.5 text-sm text-indigo-600 dark:text-indigo-400">{s.tvq>0?money2(s.tvq):'—'}</td>
                        <td className="px-3 py-2.5 text-center">{s.exceptions>0?<span className="text-sm font-bold text-red-500">⚠️ {s.exceptions}</span>:<span className="text-slate-300 dark:text-slate-700">—</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

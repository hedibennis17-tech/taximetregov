    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 shadow-sm">
          <div className="text-[9px] font-bold text-slate-400 uppercase mb-2">Cycle obligation fiscale</div>
        ...
        {nextObl&&(
          <div className="bg-blue-600 text-white rounded-2xl p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-[9px] font-bold opacity-80 mb-1">📅 PROCHAINE OBLIGATION</div>
                ...
                <div className="text-[10px] opacity-75 mt-1">Échéance : {nextObl.due} · PILOTE</div>
              </div>
              <div className="text-right shrink-0">
                <div className="text-3xl font-black">{money(nextObl.amount)}</div>
                <Link href="/declarations" className="mt-2 block text-[10px] font-bold bg-white text-blue-600 px-3 py-1.5 rounded-xl hover:bg-blue-50 text-center">→ Préparer déclaration</Link>
              </div>
            </div>
          </div>
        )}
              ...
              <div key={o.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm" style={{borderLeft:`3px solid ${sc.color}`}}>
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    ...
                    <div className="text-[10px] font-bold text-slate-500">{o.period}</div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-[9px] text-slate-400">Échéance: {o.due}</div>
                    {o.status==='PAID'&&o.paidAt&&<div className="text-[9px] font-bold text-green-600 dark:text-green-400">✓ Payé le {o.paidAt}</div>}
                  </div>
                </div>
                ...
                {tp&&(
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-purple-50 dark:bg-purple-500/8 border border-purple-200 dark:border-purple-500/20 rounded-xl p-3">
                      <div className="text-[8px] font-bold text-purple-600 dark:text-purple-400 mb-1">TPS (5%)</div>
                      <div className="text-sm font-black text-purple-700 dark:text-purple-300">{money2(tp.tpsCollected)}</div>
                      <div className="text-[8px] text-slate-400">sur {money(tp.gross)} bruts</div>
                    </div>
                    <div className="bg-indigo-50 dark:bg-indigo-500/8 border border-indigo-200 dark:border-indigo-500/20 rounded-xl p-3">
                      <div className="text-[8px] font-bold text-indigo-600 dark:text-indigo-400 mb-1">TVQ (9,975%)</div>
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                  {o.status==='UPCOMING'&&(
                    <>
                      <Link href="/declarations" className="px-3 py-1.5 rounded-xl text-[9px] font-bold bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100">📋 Préparer déclaration</Link>
                      <Link href="/fiscal" className="px-3 py-1.5 rounded-xl text-[9px] font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100">🧾 Voir fiscal</Link>
                    </>
                  )}
                  {o.status==='PAID'&&(
                    <Link href="/payments" className="px-3 py-1.5 rounded-xl text-[9px] font-bold bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 text-green-600 dark:text-green-400 hover:bg-green-100">✅ Voir paiement</Link>
                  )}

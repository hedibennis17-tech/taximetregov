    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-5xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 shadow-sm">
          <div className="text-[9px] font-bold text-slate-400 uppercase mb-2">Architecture fournisseur → revenus</div>
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span className="text-[11px] font-bold text-slate-800 dark:text-slate-200">{p.name}</span>
                        <div className={`w-1.5 h-1.5 rounded-full ${sc.dot}`}/>
                      </div>
                      <div className="text-[8px] text-slate-400">{activeCount}/{psvcs.length} services actifs · {p.country}</div>
                    </div>
            ...
            {selPrv&&(
              <>
                  ...
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-full" style={{color:PRV_STATUS[selPrv.status]?.color,background:PRV_STATUS[selPrv.status]?.bg}}>{PRV_STATUS[selPrv.status]?.label}</span>
                        <span className="text-[8px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-1.5 py-0.5 rounded-full">DEMO</span>
                      </div>
                      <div className="text-[10px] text-slate-400">{selPrv.id} · {selPrv.country} · {selSvcs.length} service(s)</div>
                      <div className="text-[9px] text-slate-500 italic mt-0.5">{selPrv.note}</div>
                    </div>
                    {selPrv.status==='CONNECTED'&&(
                      <button className="px-3 py-2 rounded-xl text-[9px] font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 cursor-pointer shrink-0">Gérer · DEMO</button>
                    )}
                    {selPrv.status==='PLANNED'&&(
                      <button className="px-3 py-2 rounded-xl text-[9px] font-bold bg-qc-blue text-white cursor-pointer shrink-0 hover:bg-blue-700">Connecter · DEMO</button>
                    )}
                  </div>
                  {conn&&(
                    <div className="bg-green-50 dark:bg-green-500/8 border border-green-200 dark:border-green-500/15 rounded-xl px-3 py-2 text-[9px] text-green-700 dark:text-green-400">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
                  <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 text-[11px] font-bold text-slate-800 dark:text-white">
                      ...
                      <div key={svc.id} className="flex items-start gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50"
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-0.5">
                            <span className="text-[10px] font-bold text-slate-800 dark:text-slate-200">{svc.name}</span>
                            <span className="text-[7px] font-bold px-1.5 py-0.5 rounded-full" style={{color:cc.color,background:`${cc.color}15`}}>{cc.icon} {cc.label}</span>
                            <span className={`text-[7px] font-bold px-1.5 py-0.5 rounded-full ${isActive?'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/10':'text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-500/10'}`}>
                              {isActive?'ACTIF':'PLANIFIÉ'}
                            </span>
                          </div>
                          <div className="text-[9px] text-slate-400">{svc.desc}</div>
                          {svc.accountRef&&<div className="text-[8px] font-mono text-slate-400 mt-0.5">Réf: {svc.accountRef}</div>}
                        </div>
                        {isActive?(
                          <div className="text-right shrink-0">
                            <div className="text-sm font-black text-green-600 dark:text-green-400">{money(svc.gross)}</div>
                            {svc.tips>0&&<div className="text-[8px] text-slate-400">+{money2(svc.tips)} tips</div>}
                            <div className="text-[8px] text-slate-400">{svc.txCount} TX</div>
                            {svc.lastSync&&<div className="text-[7px] font-mono text-slate-400">{fmtDt(svc.lastSync)}</div>}
                          </div>
                        ):(
                          <div className="text-[8px] text-purple-600 dark:text-purple-400 shrink-0">🔮 Futur</div>
                        )}
                      </div>
                    )
                  })}
                ...
                {selPrv.status==='CONNECTED'&&selSvcs.some(s=>s.gross>0)&&(
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm">
                    <div className="text-[10px] font-bold text-slate-700 dark:text-slate-300 mb-3">Financier DEMO — {selPrv.name}</div>
                ...
                {selPrv.status==='PLANNED'&&(
                  <div className="bg-purple-50 dark:bg-purple-500/8 border border-purple-200 dark:border-purple-500/20 rounded-2xl p-4">
                    <div className="text-[10px] font-bold text-purple-700 dark:text-purple-400 mb-1">🔮 Intégration planifiée</div>
                    <div className="text-[9px] text-slate-600 dark:text-slate-300">

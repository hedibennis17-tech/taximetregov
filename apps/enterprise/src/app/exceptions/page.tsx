    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 shadow-sm">
          <div className="text-[9px] font-bold text-slate-400 uppercase mb-2">Processus de résolution</div>
                ...
                <div className="flex items-start justify-between gap-3 p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50" onClick={()=>setSelected(isOpen?null:exc.id)}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[9px] font-mono text-slate-400">{exc.id}{exc.txId?` · ${exc.txId}`:''}</div>
                      <div className="text-[9px] text-slate-500 mt-0.5">{drv?.name??exc.driverId??'—'} · {exc.provider} · {fmtDt(exc.at)}</div>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    {exc.diff>0&&<div className="text-lg font-black text-red-500">Δ {money2(exc.diff)}</div>}
                    <div className="text-[8px] text-slate-400 mt-1">{isOpen?'▲ Fermer':'▼ Détails'}</div>
                  </div>
                </div>
                ...
                {isOpen&&(
                  <div className="border-t border-slate-100 dark:border-slate-800 p-4 space-y-3">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                      <div className="text-[9px] font-bold text-slate-500 uppercase mb-1">Description</div>
                      <div className="text-[10px] text-slate-700 dark:text-slate-300 leading-relaxed">{exc.desc}</div>
                    </div>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-500/8 border border-blue-200 dark:border-blue-500/20 rounded-xl p-3">
                      <div className="text-[9px] font-bold text-blue-700 dark:text-blue-400 mb-1">🔍 Action requise</div>
                      <div className="text-[9px] text-slate-600 dark:text-slate-300">{exc.action}</div>
                    </div>

                    {/* Résolution si fermé */}
                    {exc.resolution&&(
                      <div className="bg-green-50 dark:bg-green-500/8 border border-green-200 dark:border-green-500/20 rounded-xl p-3">
                        <div className="text-[9px] font-bold text-green-700 dark:text-green-400 mb-1">✅ Résolution</div>
                        <div className="text-[9px] text-slate-600 dark:text-slate-300">{exc.resolution}</div>
                      </div>
                    )}
                    ...
                    {exc.status!=='CLOSED'&&(
                      <div className="flex gap-2">
                        <button className="px-3 py-1.5 rounded-xl text-[9px] font-bold bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-600 dark:text-blue-400 cursor-pointer">Ajouter justification · DEMO</button>
                        <button className="px-3 py-1.5 rounded-xl text-[9px] font-bold bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 text-green-600 dark:text-green-400 cursor-pointer">Marquer résolu · DEMO</button>
                        <Link href="/audit" className="px-3 py-1.5 rounded-xl text-[9px] font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">🛡️ Audit</Link>
                      </div>
                    )}
                  </div>
                )}

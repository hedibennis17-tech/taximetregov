    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
        ...
        <div className="flex gap-1.5">
          {['Q1','Q2','Q3'].map(q=>(
            <button key={q} onClick={()=>setPeriod(q)} className="px-4 py-2 rounded-xl text-[10px] font-bold border transition-all cursor-pointer" style={{background:period===q?'#003DA5':'transparent',color:period===q?'white':'#64748B',borderColor:period===q?'#003DA5':'rgba(148,163,184,0.30)'}}>
              {q} 2026
            </button>
          ))}
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm" style={{borderTop:`3px solid ${PERIOD_STATUS[current.status]?.color??'#003DA5'}`}}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="text-[10px] text-slate-400">{fmtDate(current.start)} → {fmtDate(current.end)}</div>
              </div>
              <span className="text-[9px] px-2.5 py-1.5 rounded-full font-bold" style={{color:PERIOD_STATUS[current.status]?.color,background:PERIOD_STATUS[current.status]?.bg}}>
              ...
              <div className="bg-purple-50 dark:bg-purple-500/8 border border-purple-200 dark:border-purple-500/20 rounded-2xl p-4">
                ...
                {[
                  ...
                  <div key={r.l} className={`flex justify-between py-1 border-b border-purple-100 dark:border-purple-500/15 last:border-0 ${r.bold?'mt-1 pt-2':''}` }>
                    <div>
                      <div className={`text-[9px] ${r.bold?'font-black text-purple-800 dark:text-purple-300':'text-slate-600 dark:text-slate-400'}`}>{r.l}</div>
                      {r.note&&<div className="text-[7px] text-slate-400 italic">{r.note}</div>}
                    </div>
                    <span className={`text-[10px] ${r.bold?'font-black text-purple-700 dark:text-purple-400':'font-semibold text-slate-800 dark:text-slate-200'}`}>{r.v}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-indigo-50 dark:bg-indigo-500/8 border border-indigo-200 dark:border-indigo-500/20 rounded-2xl p-4">
                ...
                {[
                  ...
                  <div key={r.l} className={`flex justify-between py-1 border-b border-indigo-100 dark:border-indigo-500/15 last:border-0 ${r.bold?'mt-1 pt-2':''}`}>
                    <div>
                      <div className={`text-[9px] ${r.bold?'font-black text-indigo-800 dark:text-indigo-300':'text-slate-600 dark:text-slate-400'}`}>{r.l}</div>
                      {r.note&&<div className="text-[7px] text-slate-400 italic">{r.note}</div>}
                    </div>
                    <span className={`text-[10px] ${r.bold?'font-black text-indigo-700 dark:text-indigo-400':'font-semibold text-slate-800 dark:text-slate-200'}`}>{r.v}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="text-[9px] text-slate-400 mt-0.5">TPS nette + TVQ nette · ESTIMATION PILOTE</div>
                </div>
              ...
              <tbody>
                {TAX_PERIODS.map(tp=>(
                  <tr key={tp.id} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-4 py-2.5 text-[10px]">{tp.tpsPaid>0?money2(tp.tpsPaid):<span className="text-amber-500">À payer</span>}</td>
                    <td className="px-4 py-2.5 text-[10px]">{tp.tvqPaid>0?money2(tp.tvqPaid):<span className="text-amber-500">À payer</span>}</td>
                    <td className="px-4 py-2.5"><span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full ${tp.status==='PAID'?'text-green-600 bg-green-50 dark:text-green-400 dark:bg-green-500/10':tp.status==='OPEN'?'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-500/10':'text-red-500 bg-red-50 dark:bg-red-500/10'}`}>{tp.status==='PAID'?'Clôturée':tp.status==='OPEN'?'En cours':'En retard'}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {nextObl&&(
          <div className="bg-blue-50 dark:bg-blue-500/8 border border-blue-200 dark:border-blue-500/20 rounded-2xl p-4 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-[9px] text-slate-400">Échéance: {nextObl.due}</div>
            </div>
            <Link href="/obligations" className="px-4 py-2 rounded-xl text-[10px] font-bold bg-blue-600 text-white hover:bg-blue-700 shrink-0">→ Obligations</Link>
          </div>
        )}

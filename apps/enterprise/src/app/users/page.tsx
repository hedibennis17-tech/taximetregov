  ...
  return (
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
        ...
        <div className="text-[9px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-3 py-2 rounded-xl">{PILOT} · Isolation stricte des données par entreprise</div>

        <div className="flex gap-1.5 flex-wrap">
          {TABS.map(t=>(
            <button key={t} onClick={()=>setTab(t)} className="px-4 py-2 rounded-xl text-[10px] font-bold border transition-all cursor-pointer" style={{background:tab===t?'#003DA5':'transparent',color:tab===t?'white':'#64748B',borderColor:tab===t?'#003DA5':'rgba(148,163,184,0.30)'}}>
              {t}
            </button>
          ))}
        </div>
        ...
        {tab==='Utilisateurs'&&(
          <div className="space-y-2">
            ...
            <div className="bg-blue-50 dark:bg-blue-500/8 border border-blue-200 dark:border-blue-500/20 rounded-xl p-3 text-center">
              <button className="text-[10px] font-bold text-blue-600 dark:text-blue-400 cursor-pointer">+ Inviter un utilisateur (DEMO)</button>
              <div className="text-[8px] text-slate-400 mt-1">Invitation → Email → Acceptation → Rôle → Accès · PILOTE</div>
            </div>
          </div>
        )}
              ...
              {Object.entries(ROLE_CONF).map(([key,rc])=>(
                <div key={key} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-bold text-slate-800 dark:text-slate-200">{rc.label}</span>
                  </div>
                  <div className="flex gap-1 flex-wrap">
                    {rc.perms.map(p=><span key={p} className="text-[7px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{p}</span>)}
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                    <th className="px-4 py-2.5 text-left text-[9px] font-bold text-slate-400 uppercase">Ressource</th>
                    {['OWNER','FINANCE','COMPLIANCE','DISPATCH','VIEWER'].map(r=>(
                      <th key={r} className="px-3 py-2.5 text-center text-[8px] font-bold whitespace-nowrap" style={{color:ROLE_CONF[r]?.color??'#64748B'}}>{ROLE_CONF[r]?.label??r}</th>
                    ))}
                  </tr></thead>
                  <tbody>
        ...
        {tab==='Sécurité'&&(
          <div className="space-y-3">
              ...
              {[
                ...
                <div key={r.l} className="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[8px] font-bold ${r.ok?'text-green-600 dark:text-green-400':'text-red-500'}`}>{r.ok?'✓':'✗'}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-[9px] text-slate-400 text-center">{PILOT} · Aucun système d'authentification gouvernemental réel connecté</div>
          </div>
        )}
      </div>
    </AppShell>

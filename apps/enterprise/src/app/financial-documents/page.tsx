    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-5xl mx-auto">
          ...
          <div className="space-y-0">
            {[
              ...
              <div key={r.l} className={`flex justify-between items-start py-2 border-b border-slate-100 dark:border-slate-800 last:border-0 ${'bold' in r && r.bold?'bg-slate-50 dark:bg-slate-800 px-3 rounded-xl -mx-3 mt-1':''}`}>
                <div>
                  <div className={`text-[10px] ${'bold' in r && r.bold?'font-black':'font-semibold'} text-slate-800 dark:text-slate-200`}>{r.l}</div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
          ...
          <div className="text-[9px] text-slate-400 mb-4">Avr–Sep 2026 · Taxi + Rideshare + Livraison · DONNÉES DEMO</div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={()=>setTypeF('ALL')} className="px-2.5 py-1.5 rounded-xl text-[9px] font-bold border transition-all cursor-pointer" style={{background:typeF==='ALL'?'#003DA5':'transparent',color:typeF==='ALL'?'white':'#64748B',borderColor:typeF==='ALL'?'#003DA5':'rgba(148,163,184,0.30)'}}>Tous types</button>
            {types.map(t=>{
              const tc = FD_TYPE_CONF[t]!
              return (
                <button key={t} onClick={()=>setTypeF(t)} className="px-2.5 py-1.5 rounded-xl text-[9px] font-bold border transition-all cursor-pointer" style={{background:typeF===t?tc.color:'transparent',color:typeF===t?'white':'#64748B',borderColor:typeF===t?tc.color:'rgba(148,163,184,0.30)'}}>
                  {tc.icon} {tc.label}
                </button>
              )
            })}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 flex justify-between">
            ...
            <span className="text-[8px] font-bold text-slate-400">Versionnement · Aucune suppression silencieuse</span>
          </div>
          {filtered.map(doc=>{
              ...
              <div key={doc.id} className="flex items-start gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50" style={{borderLeft:`3px solid ${tc.color}`}}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className="text-[7px] font-bold px-1.5 py-0.5 rounded" style={{color:tc.color,background:`${tc.color}15`}}>{tc.label}</span>
                    {doc.version>1&&<span className="text-[7px] font-bold bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 px-1.5 py-0.5 rounded">V{doc.version}</span>}
                  </div>
                  <div className="text-[9px] font-mono text-slate-400 mb-0.5">{doc.id} · {doc.period} · Source: {doc.source}</div>
                  ...
                  <div className="text-[8px] text-slate-400 mt-0.5">{fmtDt(doc.at)} · Par: {doc.by}</div>
        ...
        <div className="bg-blue-50 dark:bg-blue-500/8 border border-blue-200 dark:border-blue-500/20 rounded-2xl p-4">
          <div className="text-[10px] font-bold text-blue-700 dark:text-blue-400 mb-2">📋 Politique de versionnement</div>
          <div className="text-[9px] text-slate-600 dark:text-slate-300 space-y-1">

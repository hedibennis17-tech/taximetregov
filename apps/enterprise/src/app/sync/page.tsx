    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="text-[8px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-1.5 py-0.5 rounded-full">PILOTE</span>
              </div>
              <div className="text-[10px] text-slate-400">Dernière sync: {fmtDt(lastSync.at)} · Source: {lastSync.source} · {lastSync.records.toLocaleString('fr-CA')} enregistrements</div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-sm font-black text-green-600 dark:text-green-400">{lastSync.records.toLocaleString('fr-CA')}</div>
              <div className="text-[9px] text-slate-400">enreg. vérifiés</div>
            </div>
          </div>
          {/* Anti-doublon */}
          <div className="bg-blue-50 dark:bg-blue-500/8 border border-blue-100 dark:border-blue-500/15 rounded-xl px-3 py-2">
            <div className="text-[9px] font-bold text-blue-700 dark:text-blue-400 mb-0.5">🔒 Anti-duplication actif</div>
            <div className="text-[8px] text-slate-500">Chaque activité identifiée par: <span className="font-mono font-bold">source + external_reference + timestamp</span> · Doublon détecté → ignoré + log audit</div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm">
          <div className="text-[10px] font-bold text-slate-700 dark:text-slate-300 mb-3">État des sources</div>
          {ENT_CONNECTIONS.filter(c=>c.status==='CONNECTED').map(c=>(
            <div key={c.id} className="flex items-center gap-3 py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
              ...
              <div className="flex-1">
                <div className="text-[10px] font-bold text-slate-800 dark:text-slate-200">{c.name}</div>
                <div className="text-[9px] text-slate-400">{c.dataRx.toLocaleString('fr-CA')} reçus · {c.latency}ms</div>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-16 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{width:`${c.health}%`,background:CONN_HEALTH_COLOR(c.health)}}/>
                </div>
                <span className="text-[9px] font-bold" style={{color:CONN_HEALTH_COLOR(c.health)}}>{c.health}%</span>
              </div>
            </div>
          ))}
        </div>
              ...
              <div key={h.id} className="flex items-start gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50" style={{borderLeft:`3px solid ${sc.color}`}}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className="text-[10px] font-bold text-slate-800 dark:text-slate-200">{h.source}</span>
                    <span className="text-[8px] px-1.5 py-0.5 rounded-full font-bold" style={{color:sc.color,background:sc.bg}}>{sc.label}</span>
                    <span className="text-[8px] px-1.5 py-0.5 rounded font-bold" style={{color:tc.color,background:`${tc.color}15`}}>{tc.label}</span>
                  </div>
                  <div className="text-[9px] text-slate-400">{fmtDt(h.at)} · {h.duration}ms · {h.records.toLocaleString('fr-CA')} enreg.</div>
                  ...
                  {h.note&&<div className="text-[9px] text-slate-500 italic mt-0.5">{h.note}</div>}
                </div>
              </div>
            )
          })}
        ...
        <div className="bg-blue-50 dark:bg-blue-500/8 border border-blue-200 dark:border-blue-500/20 rounded-2xl p-4">
          <div className="text-[10px] font-bold text-blue-700 dark:text-blue-400 mb-2">📋 Politique de synchronisation</div>
          <div className="space-y-1 text-[9px] text-slate-600 dark:text-slate-300">

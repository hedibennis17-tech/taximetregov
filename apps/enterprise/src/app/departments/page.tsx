    ...
    <AppShell>
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-5xl mx-auto">
        ...
        <div className="rounded-2xl p-5 shadow-sm" style={{background:'#000'}}>
          ...
          <div className="text-sm font-bold mb-3" style={{color:'rgba(255,255,255,0.45)'}}>QUÉBEC · {DEPARTMENTS.filter(d=>d.status==='ACTIVE').length} départements actifs · PILOTE</div>
          ...
          <div className="grid grid-cols-3 gap-3 mt-3 pt-3" style={{borderTop:'1px solid rgba(255,255,255,0.1)'}}>
            {[
              ...
              <div key={s.l}>
                ...
                <div className="text-sm" style={{color:'rgba(255,255,255,0.45)'}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4">
          <div className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">📋 Références publiques vérifiées</div>
            ...
            {[
              ...
              <div key={s.l} className="bg-white dark:bg-slate-900 rounded-xl p-2.5 border border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-1.5 mb-1">
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${s.badge==='PUBLIC'?'text-green-600 bg-green-50 dark:text-green-400 dark:bg-green-500/10':s.badge==='NON PUBLIÉ'?'text-slate-500 bg-slate-100 dark:bg-slate-800':'text-amber-600 bg-amber-50 dark:text-amber-400 dark:bg-amber-500/10'}`}>{s.badge}</span>
                </div>
                ...
                <div className="text-xs text-slate-400 italic mt-0.5 leading-tight">{s.note}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {DEPARTMENTS.map(d=>(
            <div key={d.id}
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${d.status==='ACTIVE'?'text-white':'text-purple-600 bg-purple-50 dark:text-purple-400 dark:bg-purple-500/10'}`} style={d.status==='ACTIVE'?{background:d.color}:{}}>{d.status}</span>
                    </div>
                    <div className="text-sm text-slate-400 leading-tight">{d.desc}</div>
                  </div>
                </div>
                {d.alerts>0&&<span className="text-sm font-bold text-red-500 bg-red-50 dark:bg-red-500/10 px-1.5 py-0.5 rounded-full shrink-0">⚠️ {d.alerts}</span>}
              </div>
              ...
              {d.status==='PLANNED'&&(
                <div className="text-sm text-purple-600 dark:text-purple-400 italic">🔮 Déploiement futur — données à venir</div>
              )}
            </div>
          ))}
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm" style={{borderTop:`3px solid ${selDept.color}`}}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="text-sm text-amber-600 dark:text-amber-400 italic">⚠️ DONNÉES SYNTHÉTIQUES — PILOTE</div>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
              <Link href="/drivers" className="px-3 py-1.5 rounded-xl text-sm font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100">👤 Chauffeurs</Link>
              <Link href="/activities" className="px-3 py-1.5 rounded-xl text-sm font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100">📍 Activités</Link>
              <Link href="/fiscal" className="px-3 py-1.5 rounded-xl text-sm font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100">🧾 Fiscal</Link>
              <Link href="/compliance" className="px-3 py-1.5 rounded-xl text-sm font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100">⚖️ Conformité</Link>
            </div>
          </div>
        )}

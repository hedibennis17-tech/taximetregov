      ...
      <div className="px-4 md:px-6 py-6 space-y-5 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-slate-400">Génération · Export · Archivage · Transmission</div>
          </div>
        </div>
        <div className="text-sm font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-3 py-2 rounded-xl">{PILOT} · Rapports générés à partir de données synthétiques · DEMO</div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          <button onClick={()=>setCatF('ALL')} className="px-2.5 py-1.5 rounded-xl text-sm font-bold border transition-all cursor-pointer" style={{background:catF==='ALL'?'#000':'transparent',color:catF==='ALL'?'white':'#64748B',borderColor:catF==='ALL'?'#000':'rgba(148,163,184,0.30)'}}>
            Tous
          </button>
          {cats.map(c=>{
            const cc = RPT_CAT_CONF[c]!
            return (
              <button key={c} onClick={()=>setCatF(c)} className="px-2.5 py-1.5 rounded-xl text-sm font-bold border transition-all cursor-pointer" style={{background:catF===c?cc.color:'transparent',color:catF===c?'white':'#64748B',borderColor:catF===c?cc.color:'rgba(148,163,184,0.30)'}}>
                {cc.label}
              </button>
            )
          })}
              ...
              <div key={r.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      ...
                      <span className="text-xs font-bold px-1.5 py-0.5 rounded-full" style={{color:cc.color,background:`${cc.color}15`}}>{cc.label}</span>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-1 flex-wrap">
                    {r.formats.map(f=>(
                      <span key={f} className="text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{f}</span>
                    ))}
                  </div>
                  <button onClick={()=>handleGenerate(r.id)} disabled={isGen} className="px-3 py-1.5 rounded-xl text-sm font-bold text-white cursor-pointer transition-all disabled:opacity-60" style={{background:isGen?'#94a3b8':cc.color}}>
                    {isGen?'Génération…':'Générer · DEMO'}
                  </button>
                </div>
                {r.lastGenAt&&<div className="text-sm text-slate-400 mt-2">Dernier: {fmtDt(r.lastGenAt)}</div>}
              </div>
            )
          })}
        </div>
            ...
            return (
              <div key={g.id} className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{g.name}</span>
                    <span className="text-sm px-1.5 py-0.5 rounded-full font-bold" style={{color:sc.color,background:sc.bg}}>{sc.label}</span>
                    <span className="text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{g.format}</span>
                  </div>
                  <div className="text-sm text-slate-400">{g.period} · {g.size} · {g.generatedBy} · {fmtDt(g.generatedAt)}</div>
                  {g.sentTo&&<div className="text-sm text-green-600 dark:text-green-400">✉️ {g.sentTo}</div>}
                </div>
                <div className="flex gap-1 shrink-0">
                  ...
                  <button className="px-2 py-1 rounded-lg text-sm font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 cursor-pointer">↓</button>
                </div>
              </div>
            )
          })}

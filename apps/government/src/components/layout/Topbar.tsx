...
export function Topbar() {
  ...
  return (
    <header className="fixed top-0 right-0 h-14 flex items-center px-4 gap-3 z-20" style={{left:"var(--sidebar-w)",background:"#003DA5",boxShadow:"0 2px 8px rgba(0,45,122,0.4)"}} style={{ left: 'var(--sidebar-w)' }}>
      ...
      <button onClick={() => setLang(lang === 'fr' ? 'en' : 'fr')} className="flex items-center gap-1 rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-[11px] font-bold transition-colors hover:border-qc-blue">
        <span className={lang === 'fr' ? 'text-qc-blue dark:text-blue-400' : 'text-slate-400'}>FR</span>
        <span className="text-slate-300 dark:text-slate-600">|</span>
        <span className={lang === 'en' ? 'text-qc-blue dark:text-blue-400' : 'text-slate-400'}>EN</span>
      </button>

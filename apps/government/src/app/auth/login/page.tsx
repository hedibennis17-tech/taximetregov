...
export default function GovernmentLoginPage() {
  ...
  const form = stage === 'login' ? (
    <form onSubmit={submitLogin} className="space-y-4">
      ...
      <button disabled={loading} type="submit" className="w-full rounded-xl bg-qc-blue py-3.5 text-sm font-bold text-white disabled:opacity-60">{loading ? 'Connexion…' : 'Se connecter'}</button>
    </form>
  ) : stage === 'activate' ? (

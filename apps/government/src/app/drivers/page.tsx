      ...
      <div className="px-4 md:px-6 pb-8 space-y-4">
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
            <div className="text-xs font-bold text-white">{filtered.length} chauffeur(s)</div>
          ...
          {filtered.map((d: AnyDriver)=>{
            ...
            return (
              <Link key={d.id} href={`/drivers/${d.id}`}
                className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-800 last:border-0 hover:bg-slate-800/50 transition-colors">
                {/* Avatar */}
                <div className="w-10 h-10 rounded-xl shrink-0 flex items-center justify-center text-sm font-bold text-white" style={{background:'#003DA5'}}>
                  {d.first_name[0]}{d.last_name[0]}
                </div>
                {/* Info */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className="text-sm font-bold text-white">{d.first_name} {d.last_name}</span>

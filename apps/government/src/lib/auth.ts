// ================================================================
// TAXIMÈTRE.GOV — AUTH MIDDLEWARE (GOVERNMENT)
// Supporte: Supabase Auth token + notre session custom
// ================================================================
    ...
    try {
      ...
      if (user && !error) {
        return {
          userId:   user.id,
          email:    user.email ?? '',
          role:     (user.user_metadata?.role as string) ?? 'GOV_ADMIN',
          driverId: null,
        }
      }
    } catch { /* continuer avec session custom */ }
  }

  // 2. Fallback: notre session custom (user_sessions table)
  try {
    const db = getDb()
...
export function requireGovRole(ctx: AuthContext): Response | null {
  ...
  // En démo: accepter tous les rôles gov
